 ...
 */
	public function add() {
		...
		$cars = $this->Card->Car->find('list');
		$employees = $this->Card->Employee->find('list');
		$customers = $this->Card->Customer->find('list');
		$statuses = $this->Card->Status->find('list');
		$users = $this->Card->User->find('list');
		$this->set(compact('cars', 'employees', 'customers', 'statuses', 'users'));
	}
